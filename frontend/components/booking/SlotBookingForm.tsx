...
export default function SlotBookingForm() {
  ...
  if (bookingSuccess) {
    return (
      <div className="bg-gradient-to-br from-kisan-900 via-kisan-800 to-kisan-950 border-2 border-kisan-700 p-8 rounded-3xl shadow-2xl text-center space-y-4 max-w-xl mx-auto my-6 text-white">
        <div className="w-16 h-16 bg-kisan-700 text-white rounded-full flex items-center justify-center mx-auto shadow-lg">
          <CheckCircle2 className="w-10 h-10" />
        </div>
        <h2 className="text-2xl font-black text-white">{t.booking.success_title}</h2>
        <p className="text-xs text-kisan-200">Government Procurement Gate Access Issued</p>

        <div className="bg-kisan-950/80 border border-kisan-700/60 p-6 rounded-2xl text-left space-y-3 text-xs">
          <div className="flex justify-between border-b border-kisan-700/40 pb-2">
            <span className="text-kisan-200">{t.booking.token_number}:</span>
            <span className="font-mono font-bold text-amber-300 text-base">{bookingSuccess.token_number}</span>
          </div>
          <div className="flex justify-between border-b border-kisan-700/40 pb-2">
            <span className="text-kisan-200">{t.booking.booking_date}:</span>
            <span className="font-bold text-white">{bookingSuccess.booking_date}</span>
          </div>
          <div className="flex justify-between border-b border-kisan-700/40 pb-2">
            <span className="text-kisan-200">{t.booking.arrival_window}:</span>
            <span className="font-bold text-kisan-300">{bookingSuccess.slot_start_time} - {bookingSuccess.slot_end_time}</span>
          </div>
          <div className="flex justify-between border-b border-kisan-700/40 pb-2">
            <span className="text-kisan-200">{t.booking.crop_name}:</span>
        ...
        <button
          ...
          className="w-full py-3 bg-kisan-700 hover:bg-kisan-600 text-white font-bold rounded-xl text-xs transition-all shadow-lg"
        >
          Book Another Token
        </button>
      </div>
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-gradient-to-br from-kisan-900 via-kisan-800 to-kisan-950 text-white border-2 border-kisan-700 p-8 rounded-3xl shadow-2xl space-y-6 max-w-xl mx-auto my-6">
      <div className="flex items-center gap-3">
        <div className="bg-gradient-to-tr from-amber-400 to-kisan-400 p-2.5 rounded-2xl text-slate-900 shadow">
          <Sprout className="w-6 h-6" />
        </div>
        <div>
          <h2 className="text-xl font-black text-white">{t.booking.title}</h2>
          <p className="text-xs text-kisan-200 font-medium">Guaranteed MSP Token Allocation</p>
        </div>
      </div>

      {errorMsg && (
        <div className="p-3.5 bg-rose-950/80 border border-rose-600 rounded-xl text-rose-100 text-xs flex items-center gap-2 font-semibold">
      ...
      <div>
        ...
        <select
          ...
          required
          className="w-full bg-kisan-950/90 border-2 border-kisan-700 rounded-xl py-3 px-4 text-xs text-white focus:outline-none focus:border-kisan-400 font-semibold"
        >
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <input
            ...
            className="w-full bg-kisan-950/90 border-2 border-kisan-700 rounded-xl py-2.5 px-3 text-xs text-white focus:outline-none focus:border-kisan-400 font-bold"
          />
        </div>
      ...
      <button
        ...
        className="w-full py-3.5 bg-kisan-700 hover:bg-kisan-600 text-white font-black text-sm rounded-xl transition-all shadow-xl disabled:opacity-50"
      >
